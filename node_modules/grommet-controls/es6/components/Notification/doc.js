...
export default (function (Element) {
  var DocumentedElement = describe(Element).description('A Box to display notification messages.').usage('$ npm install grommet-controls\n \n    import { Notification } from \'grommet-controls\';\n\n    <Notification message={...} onClose={...} />');
  DocumentedElement.propTypes = {
    a11yTitle: a11yTitlePropType,
    status: PropTypes.oneOf(['ok', 'info', 'warning', 'error', 'unknown', 'disabled']).description('Status color.'),
    size: PropTypes.oneOf(['small', 'medium', 'large']).description('The font size of the notification message.'),

    state: PropTypes.string.description('State label.'),
    strong: PropTypes.bool.description('Heading bold state.'),
    message: PropTypes.string.description('Message to be displayed.').isRequired,
    icon: PropTypes.oneOfType([PropTypes.bool, PropTypes.element]).description('Notification icon.'),
    closer: PropTypes.oneOfType([PropTypes.bool, PropTypes.element]).description('Closer icon.'),
    onClose: PropTypes.func.description('Function that will be called when the user closes the notification.'),
