...
export var colorCss = function colorCss(color) {
  return css(['color:', ';svg{stroke:', ';fill:', ';transition:none;};'], color, color, color);
};
...
export var backgroundStyle = function backgroundStyle(background, theme) {
  if ((typeof background === 'undefined' ? 'undefined' : _typeof(background)) === 'object') {
    if (background.image) {
      ...
      return css(['background:', ' no-repeat;background-position:', ';background-size:cover;', ''], background.image, background.position || 'center center', colorCss(color));
      ...
      if (rgba) {
        return css(['background-color:', ';', ''], rgba, colorCss(colorIsDark(rgba) ? theme.global.colors.darkBackground.text : theme.global.colors.text));
      }
    } else if (background.dark === false) {
      return css(['color:', ';'], theme.global.colors.text);
    } else if (background.dark) {
      return css(['', ';'], colorCss(theme.global.colors.darkBackground.text));
    }
    return undefined;
  }
  if (background) {
    ...
    if (_color2) {
      return css(['background-color:', ';', ''], _color2, colorCss(colorIsDark(_color2) ? theme.global.colors.darkBackground.text : theme.global.colors.text));
    }
  }
  return undefined;
};
...
export var inputStyle = css(['box-sizing:border-box;padding:', 'px;border:', ' solid ', ';border-radius:', ';outline:none;background-color:transparent;color:inherit;font:inherit;margin:0;'], function (props) {
